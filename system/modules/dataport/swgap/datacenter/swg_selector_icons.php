...
----------------------------------------------------------------------------
$Id: swg_selector_icons.php,v 1.1 2009/03/16 07:55:58 s4u Exp $
#echo(sWGdatacenterVersion)#
sWG/#echo(__FILEPATH__)#
----------------------------------------------------------------------------
NOTE_END //n*/
/**
* dataport/swgap/datacenter/swg_selector_icons.php
*
...
if (!isset ($direct_settings['datacenter_icons_per_page'])) { $direct_settings['datacenter_icons_per_page'] = 20; }
...
case "list":
{
	...
	if ((isset ($direct_settings['dsd']['dtheme']))&&($direct_settings['dsd']['dtheme']))
	{
		...
		$direct_cachedata['page_this'] = "m=dataport&s=swgap;datacenter;selector_icons&a=list&dsd=dtheme+{$g_dtheme_mode}++doid+{$g_oid}++tid+{$direct_cachedata['output_tid']}++page+".$direct_cachedata['output_page'];
	...
	{
		if ($g_dtheme_embedded) { direct_output_related_manager ("datacenter_selector_icons_list","pre_module_service_action_embedded"); }
		elseif ($g_dtheme) { direct_output_related_manager ("datacenter_selector_icons_list","pre_module_service_action"); }
		else { direct_output_related_manager ("datacenter_selector_icons_list","pre_module_service_action_ajax"); }
	...
	if ($g_continue_check)
	{
		if (!isset ($g_task_array['datacenter_selection_customized']))
		{
			$g_task_array['datacenter_selection_customized'] = "selector_icons";
		...
		{
			$direct_cachedata['output_pages'] = ceil ($g_datacenter_array['ddbdatalinker_objects'] / $direct_settings['datacenter_icons_per_page']);
			...
			$g_offset = (($direct_cachedata['output_page'] - 1) * $direct_settings['datacenter_icons_per_page']);
			$g_datacenter_entries_array = $g_datacenter_object->get_objects (1,$g_offset,$direct_settings['datacenter_icons_per_page']);
		}
		...
		{
			$g_plocation_directory = $g_datacenter_object->get_plocation ();

			if ((isset ($g_task_array['datacenter_selection_path']))&&(!$g_plocation_directory)&&(!$g_datacenter_object->is_physical ()))
			{
				if (substr ($g_task_array['datacenter_selection_path'],-1,1) != "/") { $g_task_array['datacenter_selection_path'] .= "/"; }
				$g_plocation_directory = $g_datacenter_object->get_plocation ($g_task_array['datacenter_selection_path']);
			}
			...
			if ($g_dtheme)
			{
				...
				{
					direct_output_related_manager ("datacenter_selector_icons_list","post_module_service_action_embedded");
					$direct_cachedata['output_page_url'] = "m=dataport&s=swgap;datacenter;selector_icons&a=list&dsd=dtheme+2++tid+{$direct_cachedata['output_tid']}++";
					$direct_classes['output']->oset ("datacenter_embedded","selector_icons");
				}
				else
				{
					direct_output_related_manager ("datacenter_selector_icons_list","post_module_service_action");
					$direct_cachedata['output_page_url'] = "m=dataport&s=swgap;datacenter;selector_icons&a=list&dsd=dtheme+1++tid+{$direct_cachedata['output_tid']}++";
					$direct_classes['output']->oset ("datacenter","selector_icons");
				}
			...
			{
				$direct_cachedata['output_page_url'] = "javascript:djs_dataport_{$direct_cachedata['output_tid']}_call_url0('m=dataport&amp;s=swgap;datacenter;selector_icons&amp;a=list&amp;dsd=dtheme+0++tid+{$direct_cachedata['output_tid']}++page+[page]')";
...
".(direct_output_smiley_decode ($direct_classes['output']->oset_content ("datacenter_embedded","ajax_selector_icons"))));
			}
		}
